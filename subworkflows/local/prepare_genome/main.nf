...
workflow PREPARE_GENOME {
    ...
    if (fasta.endsWith('.gz')) {
        GUNZIP_FASTA([[id:"${fasta_file.simpleName}"], fasta_file])
        ch_fasta     = GUNZIP_FASTA.out.gunzip
        ch_versions  = ch_versions.mix(GUNZIP_FASTA.out.versions)
    } else {
        ch_fasta = Channel.value([[id:"${fasta_file.simpleName}"], fasta_file])
    }
    ...
    ch_dbsnp     = Channel.value([[id:"${dbsnp_file.simpleName}"], dbsnp_file])
    ch_dbsnp_tbi = Channel.empty()

    if (dbsnp_tbi) {
        dbsnp_tbi_file = file(dbsnp_tbi, checkIfExists: true)
        ch_dbsnp_tbi   = Channel.value([[id:"${dbsnp_file.simpleName}"], dbsnp_tbi_file])
    } else {
        SORT_INDEX_DBSNP(ch_dbsnp)
        ch_dbsnp     = SORT_INDEX_DBSNP.out.vcf
        ch_dbsnp_tbi = SORT_INDEX_DBSNP.out.tbi
        ch_versions  = ch_versions.mix(SORT_INDEX_DBSNP.out.versions)
    }

    ch_known_indels     = Channel.value([[id:"known_indels"], known_indels_file])
    ch_known_indels_tbi = Channel.empty()

    if (known_indels_tbi) {
        known_indels_tbi_file = file(known_indels_tbi, checkIfExists: true)
        ch_known_indels_tbi   = Channel.value([[id:"known_indels"], known_indels_tbi_file])
    } else {
        SORT_INDEX_INDELS(
            ch_known_indels
                .flatMap { meta, reads ->
        ...
        ch_known_indels     = SORT_INDEX_INDELS.out.vcf.groupTuple()
        ch_known_indels_tbi = SORT_INDEX_INDELS.out.tbi.groupTuple()
        ch_versions         = ch_versions.mix(SORT_INDEX_INDELS.out.versions)
    }

    ch_known_snps     = Channel.value([[id:"known_snps"], known_snps_file])
    ch_known_snps_tbi = Channel.empty()

    if (known_snps_tbi) {
        known_snps_tbi_file = file(known_snps_tbi, checkIfExists: true)
        ch_known_snps_tbi   = Channel.value([[id:"known_snps"], known_snps_tbi_file])
    } else {
        SORT_INDEX_SNPS(
            ch_known_snps
                .flatMap { meta, reads ->
        ...
        ch_known_snps     = SORT_INDEX_SNPS.out.vcf.groupTuple()
        ch_known_snps_tbi = SORT_INDEX_SNPS.out.tbi.groupTuple()
        ch_versions       = ch_versions.mix(SORT_INDEX_SNPS.out.versions)
    }

    ch_vep_cache = Channel.empty()
    if (vep_cache) {
        ch_vep_cache = Channel.value([[id:"${vep_cache_version}_${vep_genome}"], file(vep_cache, checkIfExists: true)])
    } else {
        ch_ensemblvep_input = Channel.value([[id:"${vep_cache_version}_${vep_genome}"], vep_genome, vep_species, vep_cache_version])
    ...
    dict             = ch_dict              // channel: tuple(meta, dict)
